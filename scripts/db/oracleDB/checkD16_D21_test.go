...
func TestEvalD20ObjectOwners(t *testing.T) {
	manual := evalD20(D20Input{Owners: []D20Owner{{
		Owner: "APP_OWNER", ObjectCount: 12, OracleMaintained: "N", AccountStatus: "OPEN",
	}}})
	if manual.Status != StatusManual || !strings.Contains(manual.RawConfig, "objects=12") {
		t.Fatalf("custom owner result = %+v", manual)
	}

	vulnerable := evalD20(D20Input{Owners: []D20Owner{{
...
func TestManualEvidenceIsSanitized(t *testing.T) {
	result := evalD21(D21Input{Grants: []D21Grant{{
	...
	if strings.ContainsAny(result.RawConfig, "\r\n\t") ||
		!strings.Contains(result.RawConfig, "grantee=APP ADMIN") {
